...
import { getOutgoingRequests, getConnections, getIncomingRequests, sendMatchRequest } from '../api/match';
...
export default function Dashboard() {
  ...
  const handleConnect = async (userId) => {
    if (!userId || outgoingRequests.has(userId) || connections.has(userId) || sendingRequests.has(userId)) {
      return;
    }
    ...
    content = (
      <>
        ...
        <div className={styles.notificationBanner}>
          <p>
            Incoming: <strong>{incomingCount}</strong> | Outgoing: <strong>{outgoingRequests.size}</strong>
          </p>
          <a href="/requests" className={styles.bannerLink}>View Requests</a>
        </div>
              ...
              return (
                <div key={userId} className={styles.cardBlock}>
                  ...
                  <button
                    ...
                    disabled={status === 'connected' || status === 'pending' || isSending}
                    onClick={() => handleConnect(userId)}
                  >
                    {getActionLabel(status, isSending)}
                  </button>
                  {requestErrors.has(userId) && (
                    <p className={styles.requestError}>{requestErrors.get(userId)}</p>
                  )}
                </div>
              );
            })}
